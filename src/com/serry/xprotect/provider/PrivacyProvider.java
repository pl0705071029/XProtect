...
import com.serry.xprotect.ui.SharedPreferencesEx;
...
public class PrivacyProvider extends ContentProvider {
	private static final String AUTHORITY = "com.serry.xprotect.provider";
	...
	public static void fixFilePermissions() {
		...
		if (files != null)
			for (File file : files)
				if (file.getName().startsWith("com.serry.xprotect.provider.") && file.getName().endsWith(".xml")
	...
	private static void convertRestrictions(Context context) throws IOException {
		File source = new File(Util.getUserDataDirectory(Process.myUid()) + File.separator + "shared_prefs"
				+ File.separator + "com.serry.xprotect.provider.xml");
